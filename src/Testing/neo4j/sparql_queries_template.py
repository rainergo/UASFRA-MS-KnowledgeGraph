...
WITH
    "PREFIX wd: <http://www.wikidata.org/entity/>
             SELECT ?{prop} ?{new_prop}
                    WHERE {{
                        ?wikicomp  {prop_dbp_id}   wd:" + node.{prop} + " ;
                                   {new_prop_dbp_id} ?{new_prop} .
                    FILTER langMatches( lang(?{new_prop}), \"en\" )
                  }}"   
AS sparql
CALL apoc.load.jsonParams(
...
UNWIND value['results']['bindings'] as row
WITH row['{prop}']['value'] as prop_val, 
     {'collect(' if new_prop_is_list else ''}row['{new_prop}']['value']{')' if new_prop_is_list else ''} as new_prop_val
MERGE (n:{node} {{ {prop}: prop_val }})
SET n.{new_prop} = new_prop_val;
"""
